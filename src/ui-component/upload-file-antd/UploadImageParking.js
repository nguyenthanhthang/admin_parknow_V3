...
const UploadImageParking = (props) => {
  ...
  const handleChange = ({ fileList: newFileList }) => {
    ...
    Promise.all(newFileList.map((file) => getBase64(file.originFileObj)))
      ...
      .then((newFileListWithBase64) => setImageList(newFileListWithBase64));
  };
  ...
  return (
    <>
      <Upload
        ...
        multiple
      >
        {imageList.length >= 11 ? null : uploadButton}
      </Upload>
      <Modal
        open={previewOpen}
        title={previewTitle}
...
export default UploadImageParking;
